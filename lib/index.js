...
axios.post('http://localhost/js%20day23/lib/server.php?fn=add',data).then(res=>{
//  console.log(res);
if(res == 1){
  //插入成功后，关闭弹出框，刷新页面
  // console.log('success');
...
/* ******实现数据列表********* */
function list(){
  axios.get('http://localhost/js%20day23/lib/server.php?fn=sel').then(res=>{
    //将json转化为对象
    res = JSON.parse(res)
    // console.log(res);
    //遍历数据，追加到页面中
...
function delUser(id){
  ...
  $$('#userDelModal').setAttribute('userId',id);
  // console.log($$('#userDelModal'));
}
//给删除绑定事件
